...
class MITDB_test:
    ...
    You need to download both the MIT arrhythmia database from:
    https://alpha.physionet.org/content/mitdb/1.0.0/
    and needs to be placed below this directory:
    "../mit-bih-arrhythmia-database-1.0.0/"
    """
    def __init__(self):
        current_dir = pathlib.Path(__file__).resolve()
        self.mitdb_dir = str(pathlib.Path(current_dir).parents[1] /
                             'mit-bih-arrhythmia-database-1.0.0')
    ...
    def single_classifier_test(self, detector, tolerance=0):
        ...
        for record in mit_records:
            ...
            if delay > 1:
                TP, FP, FN = _tester_utils.evaluate_detector(r_peaks, anno,
                                                             delay,
                                                             tol=tolerance)
    ...
    def classifer_test_all(self, tolerance=0):
        ...
        total_results = np.zeros((total_records,
                                  4 * len(_tester_utils.det_names)), dtype=int)
        ...
        for det_name in _tester_utils.det_names:
            ...
            result = self.single_classifier_test(
                _tester_utils.det_from_name(det_name, 360),
                tolerance=tolerance)
