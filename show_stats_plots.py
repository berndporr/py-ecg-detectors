...
def compare_det_test(results_df, detector_name1, detector_name2,
                     experiment=None):
...
def single_plot(data, std, y_label, title=None):
    fig, ax = plt.subplots()
    _tester_utils.det_fullnames = ['Elgendi et al', 'Matched Filter',
                                   'Kalidas and Tamil', 'Engzee Mod',
                                   'Christov', 'Hamilton', 'Pan and Tompkins']
    x_pos = np.arange(len(_tester_utils.det_fullnames))

    fig.set_size_inches(10, 7)
    rects1 = ax.bar(x_pos, data, yerr=std, width=0.65, align='center',
...
def double_plot(data1, std1, data2, std2, y_label, legend1, legend2,
                title=None):
    fig, ax = plt.subplots()
    _tester_utils.det_fullnames = ['Elgendi et al', 'Matched Filter',
                                   'Kalidas and Tamil', 'Engzee Mod',
    ...
    fig.set_size_inches(10, 7)
    width = 0.4
    rects1 = ax.bar(x_pos, data1, width, yerr=std1, alpha=0.5, ecolor='black',
                    capsize=10)
    rects2 = ax.bar(x_pos+width, data2, width, yerr=std2, alpha=0.5,
...
def print_detector_comparision_table(title, datafile, experiment=None):
    ...
    for detlong1 in _tester_utils.det_fullnames:
        print(detlong1, " & ", end='')
    print("\\\\")
    for det1, detlong1 in zip(_tester_utils.det_names,
                              _tester_utils.det_fullnames):
        print(detlong1, " & ", end='')
        for det2, detlong2 in zip(_tester_utils.det_names,
                                  _tester_utils.det_fullnames):
...
# GUDB
gudb_cs_results = pd.read_csv('results_GUDB_chest_strap.csv', dtype=int,
                              index_col=0)
gudb_cable_results = pd.read_csv('results_GUDB_loose_cables.csv', dtype=int,
                                 index_col=0)
...
print_detector_comparision_table("EINTHOVEN SITTING", gudb_cable_results,
                                 'sitting')
print_detector_comparision_table("CHEST STRAP SITTING", gudb_cs_results,
                                 'sitting')

print_detector_comparision_table("EINTHOVEN JOGGING", gudb_cable_results,
                                 'jogging')
print_detector_comparision_table("CHEST STRAP JOGGING", gudb_cs_results,
                                 'jogging')
...
gudb_cs_sitting_avg, gudb_cs_sitting_std = get_result(gudb_cs_results,
                                                      _tester_utils.det_names,
                                                      'sitting')
print("chest strap sitting:", gudb_cs_sitting_avg)
gudb_cable_sitting_avg, gudb_cable_sitting_std = get_result(
    gudb_cable_results,
    _tester_utils.det_names,
    'sitting')
...
gudb_cs_jogging_avg, gudb_cs_jogging_std = get_result(gudb_cs_results,
                                                      _tester_utils.det_names,
                                                      'jogging')
gudb_cable_jogging_avg, gudb_cable_jogging_std = get_result(
    gudb_cable_results,
    _tester_utils.det_names,
    'jogging')
...
double_plot(gudb_cs_sitting_avg, gudb_cs_sitting_std,
            ...
            'Sensitivity (%)', 'Chest Strap', 'Loose Cables',
            'GUDB: cable, sitting')
