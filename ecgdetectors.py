...
class Detectors:
                        ...
                        if RR_ave != 0.0:
                            if QRS[-1] - QRS[-2] > 1.5 * RR_ave:
                                ...
                                for missed_peak in missed_peaks:
                                    if missed_peak - peaks[idx[-2]] > int(0.36 * self.fs) and ma[missed_peak] > 0.5 * th:  # noqa
    ...
    def christov_detector(self, unfiltered_ecg):
        ...
        for i in range(len(MA3)):
            ...
            if QRS and i < QRS[-1] + int((2.0 / 3.0 * Rm)):
                R = 0
            elif (QRS and i > QRS[-1] + int((2.0 / 3.0 * Rm)) and
                  i < QRS[-1] + Rm):
    ...
    def engzee_detector(self, unfiltered_ecg):
        ...
        for i in range(len(low_pass)):
            ...
            if counter > neg_threshold:
                unfiltered_section = unfiltered_ecg[thi_list[-1] -
                                                    int(0.01 * self.fs):i]
                r_peaks.append(self.engzee_fake_delay +
                               np.argmax(unfiltered_section) + thi_list[-1] -
                               int(0.01 * self.fs))
                counter = 0
                thi = False
                thf = False
    ...
    def matched_filter_detector(self, unfiltered_ecg):
        ...
        """
        cd = pathlib.Path(__file__).resolve()

        if self.fs == 250:
            template_dir = cd.parent / 'templates' / 'template_250hz.csv'
            template = np.loadtxt(template_dir)
        elif self.fs == 360:
            template_dir = cd.parent / 'templates' / 'template_360hz.csv'
            template = np.loadtxt(template_dir)
        else:
...
def normalise(input_array):
    return ((input_array - np.min(input_array)) /
...
def panPeakDetect(detection, fs):
    ...
    for i in range(len(detection)):
        if i > 0 and i < len(detection) - 1:
            if detection[i - 1] < detection[i] and detection[i + 1] < detection[i]:  # noqa
                peak = i
                peaks.append(i)

                if detection[peak] > threshold_I1 and (peak-signal_peaks[-1]) > 0.3 * fs:  # noqa
                    signal_peaks.append(peak)
                    indexes.append(index)
                    SPKI = 0.125 * detection[signal_peaks[-1]] + 0.875 * SPKI
                    if RR_missed != 0:
                        if signal_peaks[-1] - signal_peaks[-2] > RR_missed:
                            missed_section_peaks = peaks[indexes[-2] + 1:indexes[-1]]  # noqa
                            missed_section_peaks2 = []
                            for missed_peak in missed_section_peaks:
                                if missed_peak - signal_peaks[-2] > min_distance and signal_peaks[-1] - missed_peak > min_distance and detection[missed_peak] > threshold_I2:  # noqa
                                    missed_section_peaks2.append(missed_peak)

                            if len(missed_section_peaks2) > 0:
                                missed_peak = missed_section_peaks2[np.argmax(detection[missed_section_peaks2])]  # noqa
