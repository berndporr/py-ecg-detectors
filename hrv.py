...
class HRV:
    ...
    def SDNN(self, rr_samples, normalise=False):
        ...
        :param rr_samples: R peak sample locations
        :type rr_samples: array_like
        :param normalise: normalise the SDNN against the average RR interval,
        defaults to False
    ...
    def SDANN(self, rr_samples, average_period=5.0, normalise=False):
        """Calculate SDANN, the standard deviation of the average RR intervals
        ...
        :param normalise: normalise the SDANN against the average RR interval,
        defaults to False
        :type normalise: bool, optional
        :return: SDANN, the standard deviation of the average RR intervals
        calculated over short periods
        :rtype: float
        """
    ...
    def RMSSD(self, rr_samples, normalise=False):
        ...
        :param rr_samples: R peak sample locations
        :type rr_samples: array_like
        :param normalise: normalise the RMSSD against the average RR interval,
        defaults to False
        :type normalise: bool, optional
    ...
    def SDSD(self, rr_samples):
        """Calculate SDSD (standard deviation of successive differences), the
        standard deviation of the successive differences between adjacent NNs.
    ...
    def NN50(self, rr_samples):
        """Calculate NN50, the number of pairs of successive NNs that differ by
        more than 50 ms.
    ...
    def pNN50(self, rr_samples):
        """Calculate pNN50, the proportion of NN50 divided by total number of
        NNs.
    ...
    def NN20(self, rr_samples):
        """Calculate NN20, the number of pairs of successive NNs that differ by
        more than 20 ms.
    ...
    def pNN20(self, rr_samples):
        """Calculate pNN20, the proportion of NN20 divided by total number of
        NNs.
    ...
    def add_rr_error(self, rr_samples, error):
        ...
        error_values = np.random.randint(-abs(error), abs(error) + 1,
                                         len(rr_samples))
    ...
    def fAnalysis(self, rr_samples):
        """
        Frequency analysis to calc self.lf, self.hf, returns the LF/HF-ratio
        and also calculates the spectrum as pairs of
        (self.f_hr_axis, self.f_hr). The input arrary is in sample points where
        R peaks have been detected.
        """
        # discrete timestamps
