...
det_names = ['two_average', 'matched_filter', 'swt', 'engzee', 'christov',
             'hamilton', 'pan_tompkins']
det_fullnames = ['Elgendi', 'Matched', 'Kalidas', 'Engzee', 'Christov',
                 'Hamilton', 'Pan']


def sort_MIT_annotations(ann):
    beat_labels = ['N', 'L', 'R', 'B', 'A', 'a', 'J', 'S', 'V', 'r', 'F', 'e',
                   'j', 'n', 'E', '/', 'f', 'Q', '?']
...
def evaluate_detector(test, annotation, delay, tol=0):
    ...
    for anno_value in test:
        test_range = np.arange(anno_value - tol - delay,
                               anno_value + 1 + tol - delay)
